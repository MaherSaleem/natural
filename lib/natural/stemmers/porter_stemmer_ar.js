...
class LightStemmer {
  ...
  replacePrefixArray(lengthLargerThan, prefixesArray, hasReplaced = false) {
    let wordLength = this.word.length;
    if (hasReplaced) {
      return true;
    }
    if (wordLength <= lengthLargerThan) {
      return false;
    }
    let replaced = false;
    //TODO fix that
    for (let prefixMap of prefixesArray) {// for example word.length > 5
  ...
  stem() {
    this.word = Normalize_pre(this.word)
    ...
    if (this.is_verb) {
      ...
      // token = Suffix_Verb_Step2c(token);
      // token = Suffix_Verb_Step2b(token);
      // token = Suffix_Verb_Step2a(token);
      // token = Suffix_Verb_Step2a(token);
    ...
    return Normalize_post(this.word);
