...
class LightStemmer {
  ...
  stem() {
    ...
    this.word = Normalizer.Normalize_pre(this.word)
    this.setPossibleType(this.word);//TODO check the order of that(it should be above Normalize_pre? as original code)

    let flag = false;
    if (this.is_verb) {
      if (!this.Suffix_Verb_Step1()) {
        if (!this.Suffix_Verb_Step2a) {
          flag = this.Suffix_Verb_Step2c();//TODO check "or next" the sbl file
        }
      }
      if (!flag) {
        if (!this.Suffix_Verb_Step2b()) {
          flag = this.Suffix_Verb_Step2a();
        }
      }
      if (!flag) {
        this.Suffix_Verb_Step2a();
      }
    } else if (this.is_noun) {
      if (!this.Suffix_Noun_Step2c2) {
        flag = !this.is_defined && this.Suffix_Noun_Step1a() && (this.Suffix_Noun_Step2a || this.Suffix_Noun_Step2b || this.Suffix_Noun_Step2c1);
      }
      if (!flag) {
        (this.Suffix_Noun_Step1b() && (this.Suffix_Noun_Step2a() || this.Suffix_Noun_Step2b() || this.Suffix_Noun_Step2c1()) || (!this.is_defined && this.Suffix_Noun_Step2a())) && this.Suffix_Noun_Step3()
      }
    }

    this.Prefix_Step1();
    if (!this.Prefix_Step2a()) {
      this.Prefix_Step2b();
    }
    if (!this.Prefix_Step3a_Noun()) {
      if (this.is_noun) {
        this.Prefix_Step3b_Noun();
        this.Prefix_Step3c_Noun();//TODO double check that behaviour, it has try in the orignal code
      }
      if (this.is_verb) {
        this.Prefix_Step3_Verb();
        this.Prefix_Step4_Verb();//TODO double check that behaviour, it has try in the orignal code
      }
    }
    this.word = Normalizer.Normalize_post(this.word);
    return this.word;
  }
  ...
  setPossibleType() {
    ...
    //Checks2 (backward check)
    if ((wordLength > 5 && (word.endsWith(`${t}${a}${t}`) || word.endsWith(`${t}${y}${t}`))) //like معلمتان معلمنتين
   ...
   */
  replacePrefix(word, prefixes, replaceWith) {
  ...
  Prefix_Step1() {
    return this.replacePrefixArray(5, [
      {prefixes: [`${ao}${ao}`, `${ao}${wo}`], map: `${ao}`},
      {prefixes: [`${ao}${aM}`], map: `${aM}`},
      {prefixes: [`${ao}${wo}`], map: `${ao}`},
      {prefixes: [`${ao}${ao_}`], map: `${ao_}`},

    ]);
  }

  Prefix_Step2a() {
    return this.replacePrefixArray(5, [{prefixes: [`${f}${a}${l}`, `${w}${a}${l}`], map: ''},]);//remove فالـ والـ
  }

  Prefix_Step2b() {
    //TODO check if we can combine that with Prefix_Step2a
    return this.replacePrefixArray(5, [{prefixes: [`${f}`, `${w}`], map: ''},]);
  }

  Prefix_Step3a_Noun() {
    let replaced = this.replacePrefixArray(5, [{prefixes: [`${b}${a}${l}`, `${k}${a}${l}`], map: ''},]);
    replaced = this.replacePrefixArray(4, [{prefixes: [`${l}${l}`, `${a}${l}`], map: ''},], replaced);
    return replaced;
  }

  //TODO rename to remove suffix Array
  removeSuffixArray(lengthLargerThan, toBeDeletedArray, hasBeenRemoved = false) {
    //TODO combine that with replacePrefixArray
    let wordLength = this.word.length;
    if (hasBeenRemoved) {
      return true;
    }
    if (wordLength <= lengthLargerThan) {// for example word.length > 5
      return false;
    }
    let replaced = false;
    //TODO fix that
    for (let toBeDeletedArrayElement of toBeDeletedArray) {
      [replaced, this.word] = removeSuffix(this.word, toBeDeletedArrayElement);
      if (replaced)
        return true
    }
    return false;
  ...
  replacePrefixArray(lengthLargerThan, mappingArray, hasReplaced = false) {
    let wordLength = this.word.length;
    if (hasReplaced) {
      return true;
    }
    ...
    let replaced = false;
    //TODO fix that
    for (let prefixMap of mappingArray) {
      [replaced, this.word] = this.replacePrefix(this.word, prefixMap.prefixes, prefixMap.map);
      if (replaced)
        return true
    }
    return false;
  ...
  Prefix_Step3b_Noun() { // probably  noun and defined
    //TODO check if the order of these replacement is corrent
    return this.replacePrefixArray(3, [
      {prefixes: [`${b}`], map: ''},
      {prefixes: [`${b}${b}`], map: `${b}`},
      {prefixes: [`${k}${k}`], map: `${k}`},
    ]);
  ...
  Prefix_Step3c_Noun(word) { // probably  noun and undefined
    return this.replacePrefixArray(3, [
        {prefixes: [`${b}`], map: ''}
      ]
    );
  ...
  Prefix_Step3_Verb() { // probably  noun and undefined
    return this.replacePrefixArray(4, [
        {prefixes: [`${b}`], map: ''}
      ]
    );
  }
   ...
   * @param word: string
   */
  Suffix_Verb_Step1(word) {
    let removed = this.removeSuffixArray(5, [`${k}${m}${a}`, `${e}${m}${a}`, `${k}${m}${w}`]);//remove كمو، اما، كما
    removed = this.removeSuffixArray(4, [`${n}${y}`, `${n}${a}`, `${e}${a}`, `${e}${m}`, `${e}${n}`, `${k}${m}`, `${k}${n}`], removed);
    removed = this.removeSuffixArray(3, [`${e}`, `${k}`], removed);
    return removed;
  }
   ...
   */
  Suffix_Verb_Step2a() {
    let removed = this.removeSuffixArray(5, [`${t}${m}${a}`]);//remove درستما
    removed = this.removeSuffixArray(4, [`${a}${n}`, `${w}${n}`, `${y}${n}`], removed);//remove present suffix (يدرسان، يدرسون، تدرسون)
    removed = this.removeSuffixArray(4, [`${n}${a}`, `${t}${a}`, `${t}${n}`], removed);// remove past suffix (درسنا، درستا، درستن)
    removed = this.removeSuffixArray(3, [`${a}`, `${n}`, `${y}`], removed);
    removed = this.removeSuffixArray(3, [`${t}`], removed);
    return removed;
  }

  Suffix_Verb_Step2b() {
    return this.removeSuffixArray(4, [`${t}${m}`, `${w}${a}`]);
  }

  Suffix_Verb_Step2c() {
    let removed = this.removeSuffixArray(5, [`${t}${m}${w}`]);
    removed = this.removeSuffixArray(3, [`${t}${m}${w}`], removed);
    return removed
  }


  Suffix_Noun_Step1a() {
    let removed = this.removeSuffixArray(5, [`${k}${m}${a}`, `${e}${m}${a}`]);
    removed = this.removeSuffixArray(4, [`${n}${a}`, `${k}${m}`, `${e}${a}`, `${e}${n}`, `${e}${m}`], removed);
    removed = this.removeSuffixArray(3, [`${y}`, `${k}`, `${e}`], removed);
    return removed;
  }

  Suffix_Noun_Step1b() {
    return this.removeSuffixArray(5, [`${n}`]);
  }

  Suffix_Noun_Step2a() {
    return this.removeSuffixArray(4, [`${a}`, `${y}`, `${w}`]);
  }

  Suffix_Noun_Step2b() {
    return this.removeSuffixArray(4, [`${a}${t}`]);
  }

  Suffix_Noun_Step2c1() {
    return this.removeSuffixArray(3, [`${t}`]);
  }

  Suffix_Noun_Step2c2() { // feminine t_
    let removed = this.removeSuffixArray(4, [`${t}${a}${n}`, `${t}${y}${n}`]);
    removed = this.removeSuffixArray(2, `${t_}`, removed);
    return removed;
  }

  Suffix_Noun_Step3() { // ya` nisbiya
    return this.removeSuffixArray(2, [`${y}`]);
  }
...
function stem(token) {
  let stemmer = new LightStemmer(token);
