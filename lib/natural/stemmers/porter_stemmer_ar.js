...
class LightStemmer {
  ...
  stem(){
    ...
    setPossibleType(this.word);//TODO check the order of that(it should be above Normalize_pre? as original code)

    [done, token] = Prefix_Step3b_Noun(token);
    // if(is_verb){
...
function replacePrefixArray(word, lengthLargerThan, prefixesArray) {
  let wordLength = word.length;
  for (let prefixMap of prefixesArray){
    [replaced, word] = replacePrefix(word, prefixMap.prefixes, prefixMap.map);
    if(replaced)
      return [true, word];
  }
  return [false, word];
...
function Prefix_Step3b_Noun(word) { // probably  noun and defined
  //TODO check if the order of these replacement is corrent
    ...
    // if (replaced) return word;
    return replacePrefixArray(word, 3 ,[
      {prefixes: [`${b}`], map: ''},
      {prefixes: [`${b}${b}`], map: `${b}`},
      {prefixes: [`${k}${k}`], map: `${k}`},
    ]);
...
function Prefix_Step3_Verb(word) { // probably  noun and undefined
  return replacePrefixArray(word, 4, [
      {prefixes: [`${b}`], map: ''}
    ]
  );
}
